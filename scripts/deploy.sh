...
cmd_deploy() {
  ...
  # Busca a task definition atual como base
  local CURRENT_TD
  ...
  # Gera nova task definition substituindo apenas a imagem
  local NEW_TD_JSON
...
cmd_rollback() {
  local TARGET="$1"
  [[ -z "$TARGET" ]] && err "Informe a task definition. Ex: rollback task-def-bia:2"

  # Valida que a revisão existe
